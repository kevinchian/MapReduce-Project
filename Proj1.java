...
public class Proj1{
	...
	private final double ONE = 1.0; 
		...
		LinkedList order = new LinkedList<String>(); //order of which things appear;
		while(matcher.find()){
			...
			if(word.equals(targetGram)){
				...
				if(targetExists == false){
					...
					while(!order.isEmpty()){
						d++;
						context.write(new Text((String) order.pollLast()), new DoublePair(func.f(d), ONE));
					}
				}
				else{
					while(!order.isEmpty()){
						...
						context.write(new Text((String) order.pollLast()), new DoublePair(func.f(d), ONE));
						if(!order.isEmpty()){	
						context.write(new Text((String) order.pollFirst()), new DoublePair(func.f(d), ONE));
						}
					}
				}	
			}
		...
		if(targetExists == false){
			while(!order.isEmpty()){
				context.write(new Text((String) order.poll()), new DoublePair(func.f(Double.POSITIVE_INFINITY), ONE));
			}
		}
		else{
			double d = 0;
			while(!order.isEmpty()){
				d++;
				context.write(new Text((String) order.poll()), new DoublePair(func.f(d), ONE));
			}
		}
            }
    ...
    public static class Combine1 extends Reducer<Text, DoublePair, Text, DoublePair> {
		...
		double wordCount = 0; //total appearances of the word
		double cooccurence = 0; //sum of f(d)
		for(DoublePair value : values){
			wordCount += value.getDouble2();
			cooccurence += value.getDouble1();
		}
		context.write(key, new DoublePair(cooccurence, wordCount));
            }
    }
	...
	public void map(Text word, DoublePair value, Context context)
            throws IOException, InterruptedException {
		double cooc = value.getDouble1(); //co-occurence value
		double wordCount = value.getDouble2(); //wordcount value
		double numerator = 0;
		if(cooc > 0){
			numerator = cooc * Math.pow(Math.log(cooc),3);
		}
		double rate = numerator / wordCount;
		context.write(new DoubleWritable(rate * -1.0), word);
	}
    }
    ...
    public static class Reduce2 extends Reducer<DoubleWritable, Text, DoubleWritable, Text> {
        ...
        @Override
            public void reduce(DoubleWritable key, Iterable<Text> values,
                    Context context) throws IOException, InterruptedException {
			double rate = key.get() * -1.0;
			for(Text value : values){
				if(n < N_TO_OUTPUT){
					context.write(new DoubleWritable(rate), value);
				n++;
				} 	
			}
            }
